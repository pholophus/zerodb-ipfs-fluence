...
#[marine]
pub fn dag_put(input: String, external_multiaddr: String) -> IpfsPutResult {
    log::info!("dag_put input with {:?}", input);
    let timeout = load_config().timeout;

    if Multiaddr::from_str(&external_multiaddr).is_err() {
        return Err(eyre::eyre!("invalid multiaddr: {}", external_multiaddr)).into();
    }else{
        ipfs_dag_put(input, external_multiaddr, timeout)
    }
}

#[marine]
pub fn dag_get_from(hash: String, external_multiaddr: String) -> IpfsPutResult {
    ...
    if Multiaddr::from_str(&external_multiaddr).is_err() {
        ...
        ipfs_dag_get(hash, external_multiaddr, timeout)
    }
}
...
extern "C" {
    ...
    #[link_name = "dag_put"]
    pub fn ipfs_dag_put(input: String, api_multiaddr: String, timeout_sec: u64) -> IpfsPutResult;
